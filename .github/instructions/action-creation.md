...
GitHub workflow-command annotations (`::notice::`, `::warning::`, `::error::`) surface directly on the PR/checks UI of the **repository the action runs against**. `zz_log` (from the `tomgrv/scripts` bundle, bootstrapped by the `setup-scripts` composite step above) is now GitHub-Actions-aware: inside a real Actions run (`GITHUB_ACTIONS=true`), `zz_log w "..."`/`zz_log e "..."` also emit a leading `::warning::`/`::error::` annotation line ahead of the usual colored job-log line - `zz_log i "..."` (info) never does, and outside Actions (local `dispatch.sh` use) no annotation line is emitted at all. This means `zz_log w`/`zz_log e` are now the default way to raise a warning/error, whether it's a setup problem or a finding about the analyzed repository - use `echo "::warning::..."`/`echo "::error::..."` directly only when `zz_log`'s single-line annotation doesn't fit, e.g. a multi-line report that needs percent-encoding to stay one annotation (see `check-pr-format/run.sh`'s commitlint-output case for the pattern).

- **`zz_log e "..."`/`zz_log w "..."` for anything actionable** - missing `GITHUB_TOKEN`/`REVIEWDOG_GITHUB_API_TOKEN`, a required CLI tool not found (`jq`, `gh`, `composer`, `npm`, `reviewdog`, the linter binary, ...), a bad/missing config file path, a failed clone/push/label update, a test suite failure, and so on. `zz_log e` is still followed by `exit 1` when fatal.
...
- **`::notice::` for facts about the analyzed repository** still needs a direct `echo "::notice::..." >&2` - `zz_log` has no notice-level equivalent: no files matched the target path/filter, a target directory or manifest is absent, nothing changed, a PR is already up to date.
- **Multi-line or otherwise pre-formatted `::warning::`/`::error::` content** (e.g. commitlint's percent-encoded, newline-preserving report) also stays a direct `echo` - `zz_log`'s automatic annotation prints its message verbatim on a single line, which would corrupt a report that depends on that encoding.
