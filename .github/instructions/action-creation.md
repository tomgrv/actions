...
- **`zz_log n "..."` for a *result* or *fact* about the analyzed repository** worth surfacing on the PR/checks UI but not actionable enough for a warning/error: no files matched the target path/filter, a target directory or manifest is absent, nothing changed, a PR is already up to date, a PR was created/updated/successfully rebased. Not for narrating that a step is about to run (see `zz_log i` below) - only for what it found or did.
- **`zz_log i "..."` for non-actionable informational detail** that shouldn't surface as an annotation at all: an input left at its default (`"PATHS not set, using default: app"`), a step's routine progress ("Cloning ...", "Rebasing PR #X onto Y...", "Deleting run ...").
