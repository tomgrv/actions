...
- **`zz_log n "..."` explains a success** - what the step accomplished, not actionable enough for a warning/error but still worth a PR/checks-UI notice: a PR was created/updated, an import completed, a rebase succeeded, a title passed validation, an explicit rule exception was granted (e.g. hotfix-marked). Not for narrating that a step is about to run, and not for a skip/no-op/nothing-to-do outcome (missing/empty target, no matching files, already up to date, nothing changed) - those aren't something the step accomplished, so they go to `zz_log i` instead.
- **`zz_log i "..."` for everything that isn't a success to report**: routine progress ("Cloning ...", "Rebasing PR #X onto Y..."), an input left at its default, and every skip/no-op/nothing-to-do case described above.
