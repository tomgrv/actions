...
By default `reporter` is resolved by [**setup-reviewdog**](../setup-reviewdog/README.md) for this run's context (`github-pr-check` on pull requests, `github-check` otherwise), posting findings as Check annotations that show up on every PR regardless of whether it touches `composer.json`. Switch `reporter` to `github-pr-review` to get applyable one-click suggestion boxes instead — but GitHub can only attach those to lines already part of the PR's diff, so on PRs that don't modify `composer.json` (the common case) findings fall back to a generic rolled-up comment instead of an inline suggestion.
...
**Optional.** Reporter of reviewdog command `[github-pr-check,github-check,github-pr-review]`. Defaults to the reporter resolved by [**setup-reviewdog**](../setup-reviewdog/README.md) for this run's context (`github-pr-check` on pull requests, `github-check` otherwise), which posts a Check annotation for every finding regardless of whether the PR touches `composer.json`. Requires the job to have `checks: write` permission. Set explicitly to `github-pr-review` to post applyable version-bump suggestions instead, but only for findings on lines already part of the PR diff; everything else is dropped to a generic rolled-up comment. Requires `pull-requests: write` permission.

### filter-mode

Fixed to `nofilter`: this action audits the whole dependency tree rather than specific files, so reviewdog reports every finding regardless of diff. Not configurable.
