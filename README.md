...
- **test-workspaces**: Discovers every workspace (via `list-packages`) and runs each one's `bats run.bats` suite (via `run-workspace-tests`, driven by its `package.json` `scripts.test` entry) on every PR and push to main/develop
